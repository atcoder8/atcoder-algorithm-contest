...
fn main() {
    ...
    xr.sort_unstable_by_key(|(x, r)| x - r);

    let mut uf = UnionFind::new(n);

    let mut coord_to_idx = BTreeMap::<i64, usize>::new();
    for (i, &(x, r)) in enumerate(&xr) {
        let minus = x - r;
        let plus = x + r;

        if let Some(&idx) = coord_to_idx.get(&minus) {
            uf.merge(i, idx);
        } else {
            coord_to_idx.insert(minus, i);
        }
        if let Some(&idx) = coord_to_idx.get(&plus) {
            uf.merge(i, idx);
        } else {
            coord_to_idx.insert(plus, i);
        }
    }
