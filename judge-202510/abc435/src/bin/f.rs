...
fn main() {
    ...
    let max_position = st.all_prod().max_position;
    let max_score = recursion(&pp, &st, max_position, 0, n);
    println!("{max_score}");
}

fn recursion(pp: &[usize], st: &SegmentTree<S>, curr: usize, left: usize, right: usize) -> usize {
    let mut cand_scores = vec![];
    if curr > left {
        let max_position = st.prod(left..curr).max_position;
        cand_scores.push(curr - max_position + recursion(pp, st, max_position, left, curr));
    }
    if curr + 1 < right {
        let max_position = st.prod(curr + 1..right).max_position;
        cand_scores.push(max_position - curr + recursion(pp, st, max_position, curr + 1, right));
    }
    cand_scores.into_iter().max().unwrap_or(0)
}
...
impl Monoid for S {
    ...
    fn prod(&self, rhs: &Self) -> Self {
        max_by_key(*self, *rhs, |v| v.max_height)
    }
}
