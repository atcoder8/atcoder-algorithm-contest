...
fn main() {
    ...
    let output = answers
        .into_iter()
        .map(|answer| if answer { "Yes" } else { "No" })
        .join("\n");
    println!("{output}");
}
...
fn recursive(
    ...
    current: usize,
    aa: &[u32],
    pool: &mut Multiset<u32>,
    duplicated: bool,
    answers: &mut [bool],
) {
    let a = aa[current];
    let duplicated = duplicated || pool.contains(&a);
    answers[current] = duplicated;
    pool.insert(a);
    graph[current]
        .iter()
        .filter(|&&adjacent| Some(adjacent) != parent)
        .for_each(|&child| recursive(graph, Some(current), child, aa, pool, duplicated, answers));
    pool.remove(&a);
}
