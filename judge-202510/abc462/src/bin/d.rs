...
const MAX: usize = 10_usize.pow(6) + 1;

fn main() {
    input! {
        (n, d): (usize, usize),
        mut st: [(Usize1, usize); n],
    }

    st.sort_unstable();

    let mut num_combs = 0;
    let mut ft = FenwickTree::<i64>::new(MAX + d + 1);
    let mut idx = 0;
    for time in 0..=MAX {
        while idx < n && st[idx].0 <= time {
            ft.add(st[idx].0, 1);
            ft.add(st[idx].1, -1);
            idx += 1;
        }
        let cnt = ft.sum(..=time + d);
        num_combs += cnt * cnt.saturating_sub(1) / 2;
    }
    println!("{num_combs}");
}
