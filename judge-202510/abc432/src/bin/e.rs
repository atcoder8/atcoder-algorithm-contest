...
fn main() {
    ...
    let mut count_ft = FenwickTree::<usize>::new(MAX + 1);
    let mut sum_ft = FenwickTree::<usize>::new(MAX + 1);
    for &a in &aa {
        count_ft.add(a, 1);
        sum_ft.add(a, a);
    }
    ...
    for query in queries {
        match query {
            ...
            Query::Ask { l, r } => {
                let sum = if l <= r {
                    l * count_ft.sum(..l) + sum_ft.sum(l..r) + r * count_ft.sum(r..)
                } else {
                    l * count_ft.sum(..)
                };
                println!("{sum}");
