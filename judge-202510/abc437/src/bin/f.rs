...
fn main() {
    ...
    let mut st = SegmentTree::from(xy.iter().map(|&coord| M::new(coord)).collect_vec());
...
fn rotate(coord: (i64, i64)) -> (i64, i64) {
    let (x, y) = coord;
    (x - y, x + y)
}
...
enum Query {
    Modify(usize, i64, i64),
    Ask(usize, usize, i64, i64),
}
...
impl Query {
    fn read() -> Self {
        ...
        if qt == 1 {
            input! {
                (i, x, y): (Usize1, i64, i64),
            }

            Self::Modify(i, x, y)
        } else {
            input! {
                (l, r, x, y): (Usize1, usize, i64, i64),
            }
...
impl Monoid for M {
    ...
    fn prod(&self, rhs: &Self) -> Self {
        Self {
            min_u: self.min_u.min(rhs.min_u),
            max_u: self.max_u.max(rhs.max_u),
            min_v: self.min_v.min(rhs.min_v),
            max_v: self.max_v.max(rhs.max_v),
        }
    }
}

impl M {
    fn new(coord: (i64, i64)) -> Self {
        let (u, v) = rotate(coord);
        Self {
            min_u: u,
            max_u: u,
            min_v: v,
